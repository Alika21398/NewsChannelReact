      ...
      fetch(
        ...
        .then((data) =>
          // console.log("API Response:", data); 
           setData(data.results.filter((items)=>{
          if(items.image_url)
          return items;
      })))
      .catch(error => {
    ...
    <div className="flex flex-wrap gap-10  py-4 px-20">
        ...
        {data?.map((items, index)=>{
            ...
            <div className="py-9 flex justify-between border-b " key={index}>
                ...
                <h3 className='w-[50%]'><Link to={`/details/${cid}/${items.article_id}`}>{items.title}</Link></h3>
                <div className='border border-black w-[40%] h-[250px] rounded-md'>
                <Link to={`/details/${cid}/${items.article_id}`}>
                <img className='w-full h-full object-cover'  src={items.image_url} alt="" />
                </Link>
                </div>
      ...
      <div className='w-[30%]'>
        {data?.map((items,index)=>{
            ...
            <div className='flex justify-between border-b py-7' key={index}>
                <div className='w-[45%]'><Link to={`/details/${cid}/${items.article_id}`}>{items.title}</Link></div>
